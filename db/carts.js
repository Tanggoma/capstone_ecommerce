...
async function clearCart(userId, sessionId) {
    try {
        if (userId) {
            const result = await client.query('DELETE FROM carts WHERE user_id = $1', [userId]);
            return { success: true, message: `Deleted ${result.rowCount} cart(s) for userId: ${userId}.` };
        } else if (sessionId) {
            const result = await client.query('DELETE FROM carts WHERE session_id = $1', [sessionId]);
            return { success: true, message: `Deleted ${result.rowCount} cart(s) for sessionId: ${sessionId}.` };
        } else {
            throw new Error("You must provide either a userId or sessionId.");
        }
    } catch (error) {
